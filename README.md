### Hi, I'm Anas Amejjoud! 👋  
I'm a passionate Full Stack Developer with expertise in both front-end and back-end technologies. Over the years, I've worked on diverse projects that have sharpened my skills and deepened my understanding of web and software development. I thrive on building efficient, scalable, and high-quality applications.

## 🚀 Skills & Expertise  

### 💻 Front-End Development  
- **Languages & Technologies:** HTML5, CSS3, JavaScript, jQuery  
- **Frameworks & Libraries:** Vue.js, React.js, Bootstrap  

### 🖥️ Desktop & Cross-Platform Development  
- **Technologies:** Electron.js  

### 📱 Mobile App Development  
- **Framework:** Flutter  

### 🛠️ Back-End Development  
- **Languages:** PHP, Node.js  
- **Frameworks:** Laravel  

### 🗄️ Database Management  
- **SQL:** MySQL, SQLite  
- **NoSQL:** MongoDB  

### 🔄 Version Control and DevOps  
- **Tool:** Git, GitHub, GitHub Actions 

### 🤖 AI Assisted Development  
AI powered development using Cursor and Claude to accelerate coding, debugging, and feature development.

## 🌟 Professional Experience  

### 💼 Web Developer | Yoorika Web Services, Marrakech  
- Developed **SaaS platforms** such as **Call Center**, **Doctor Management System**, and **Real Estate Management System** using **pure PHP**.  
- Focused on building scalable and efficient web applications.  

### 💼 Full Developer | Seocom, Marrakech  
- Debugged and enhanced platforms built with **Laravel**.  
- Developed web applications from scratch using **Laravel**.  
- Built a **mobile app** using **Cordova**.  

### 💼 Full Stack Developer | Helloworld Agency (Current)  
- Developing **websites** and **platforms** using **PHP & Laravel**.  
- Worked on **mobile applications**, adding new features and fixing bugs.

### 📱 Mobile Application Developer | eGrow  
Developing and maintaining the mobile application using Flutter. The application allows businesses to manage orders, customer conversations, and sales activities from mobile devices. Integrated REST APIs to synchronize real time data between the mobile app and backend services. Focused on responsive UI and performance optimization for Android devices while using AI assisted development tools such as Cursor and Claude to accelerate development and feature delivery.

## 🎓 Education  

- **Baccalaureate in Science** | Zerktouni High School, Marrakech (2016-2017)  
- **Diploma in Computer Development** | (2017-2019)  
- **Self-Learning & Continuous Improvement**  
  - Actively learning through online platforms to enhance skills and stay up-to-date with the latest technologies. 

### 📚 Languages:

Arabic (Native), English (Intermediate).

## 🏆 Interests  

- I enjoy learning new technologies and improving my skills, especially in **Laravel packages**, **code optimization**, and **problem-solving**.  
- Always eager to explore innovative solutions and stay updated with the latest industry trends.  

Feel free to explore my repositories and reach out for **collaborations** or **tech discussions**! 🚀  

<h2> My Skills</h2>

[![My Skills](https://skillicons.dev/icons?i=js,html,css,jquery,vue,laravel,react,flutter,bootstrap,mysql,mongodb,wordpress,git)](https://skillicons.dev)

<h2>Stats</h2>

<p align="left">
  <table>
    <tr>
      <td><img src="https://github-readme-stats.vercel.app/api?username=DevanasDeveloper&show_icons=true&theme=dark" alt="Anas's GitHub stats"/></td>
      <td><img src="https://github-readme-stats.vercel.app/api/top-langs/?username=DevanasDeveloper&layout=compact&theme=dark" alt="Top Langs"/></td>
    </tr>
  </table>
</p>


<!--
**DevanasDeveloper/DevanasDeveloper** is a ✨ _special_ ✨ repository because its `README.md` (this file) appears on your GitHub profile.

Here are some ideas to get you started:

- 🔭 I’m currently working on ...
- 🌱 I’m currently learning ...
- 👯 I’m looking to collaborate on ...
- 🤔 I’m looking for help with ...
- 💬 Ask me about ...
- 📫 How to reach me: ...
- 😄 Pronouns: ...
- ⚡ Fun fact: ...
-->
